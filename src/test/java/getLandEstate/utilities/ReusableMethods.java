...
public class ReusableMethods {
    ...
    */
    ...
    public static void screenShot(String name) {
        ...
        String dosyaYolu = System.getProperty("user.dir") + "/src/test/java/getLandEstate/testOutputs/" + name + date + ".png";
        try {
...
//webelement screenshot
    public static void   screenShotOfWebElement(WebElement webElement){
        String date = DateTimeFormatter.ofPattern("ddMMyyyy_HHmmss").format( LocalDateTime.now() );
        String dosyaYolu = System.getProperty("user.dir") + "/src/test/java/getLandEstate/testOutputs/" +  date + ".png";
        try {
    ...
    public void uploadFilePathWindows(String dosyaYolu) {
        try {
    ...
    //Mac kullanicilari icin
    public void uploadFilePathMac(String dosyaYolu) {
        try {
            waitForSecond(3); // 3 saniye bekletir. Bu, kodun başka işlemler için hazır olmasını sağlar.
            StringSelection stringSelection = new StringSelection(dosyaYolu);
            // Verilen Dosya yolunu bir StringSelection objectine dönüştürürüz
            ...
            Robot robot = new Robot();
            // Robot sınıfından bir object olustururuz, Bu class javadan gelir ve klavye ve mouse etkileşimlerini simüle eder.
            robot.keyPress(KeyEvent.VK_META); // Mac'te Command tuşu
            robot.keyPress(KeyEvent.VK_V); // CTRL+V tuslarina basar dolayisiyla panodaki veriyi yapıştırır.
            robot.keyRelease(KeyEvent.VK_META); // Command tuşundan elini kaldirir
            robot.keyRelease(KeyEvent.VK_V);
            robot.delay(3000);
            // 3 saniye bekler, bu süre içerisinde yapıştırılan verinin işlenmesini sağlar.
            robot.keyPress(KeyEvent.VK_ENTER);
